...
export default async function OccupancyRecordPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  return (
    <OperationsRecordFrame session={session} record={operationsRecord} current="occupancy" entity="unit" title="سجل الإشغال والسكن" description="تابع حالة الإشغال ومدة العلاقة والمستندات والتنبيهات المرتبطة بالوحدة، مع إخفاء بيانات التواصل الحساسة.">
      ...
      <div className={styles.footerActionRow}>
        ...
        <Link className={styles.footerLinkPrimary} href={`/operations/records/${id}/payments`}>الانتقال إلى سجل الدفعات</Link>
      </div>
