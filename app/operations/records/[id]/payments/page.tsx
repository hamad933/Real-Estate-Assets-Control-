...
export default async function PaymentsRecordPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  return (
    <OperationsRecordFrame session={session} record={operationsRecord} current="payments" surfaceCode="S09" entity="unit" title="سجل الدفعات والتحصيل" description="سجل تشغيلي محدد للدفعات المستحقة والمتابعة والتحصيل باستخدام مبالغ تركيبية ثابتة، دون مزود دفع أو تكامل محاسبي.">
      <section className={styles.paymentHero} aria-label="ملخص حالة السداد">
        ...
        <div className={styles.paymentMetric}><span>المبلغ الحالي المستحق</span><strong>{payments.dueAmount}</strong><small>بيانات تركيبية</small></div>
        ...
        <div className={styles.paymentMetric}><span>الرصيد المرتبط بالسجل</span><strong>{payments.linkedBalance}</strong><small>ليس رصيدًا محاسبيًا خارجيًا</small></div>
      </section>
      <div className={styles.twoColumn}>
        <section className={styles.panel} aria-labelledby="payment-history-title">
          <div className={styles.panelTitle}><h2 id="payment-history-title">سجل الدفعات</h2><span>حالات حتمية من SQLite المحلي</span></div>
        ...
        <section className={styles.panel} aria-labelledby="collection-title">
          <div className={styles.panelTitle}><h2 id="collection-title">التحصيل والمتابعة</h2><span>لا يوجد اتصال بخدمة خارجية</span></div>
      ...
      <div className={styles.twoColumnReverse}>
        <section className={styles.panel} aria-labelledby="payment-documents-title"><div className={styles.panelTitle}><h2 id="payment-documents-title">المستندات والأدلة</h2><span>مراجع محلية تركيبية</span></div><div className={styles.documentList}>{payments.documents.map((document) => <div className={styles.documentRow} key={document}><strong>{document}</strong><span className={styles.badgeNeutral}>موثق</span></div>)}</div></section>
