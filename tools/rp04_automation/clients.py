...
class JulesClient:
    def __init__(self, api_key: str, api_base: str = API_BASE, timeout: int = 45, opener: Callable[..., Any] | None = None):
        if not api_key: raise ValidationError("JULES_API_KEY_unavailable")
    ...
    def _request(self, method: str, path: str, body: dict[str, Any] | None = None, *, read_retry: bool = False) -> tuple[int, dict[str, Any]]:
        ...
        for attempt in range(attempts):
            ...
            try:
                ...
                except json.JSONDecodeError: payload = {"error_text": raw[:2000]}
                ...
                if read_retry and attempt + 1 < attempts:
                    time.sleep(2 ** attempt); continue
                if method != "GET": raise UnknownWriteOutcome(type(exc).__name__) from exc
    ...
    def _read(self, path: str) -> dict[str, Any]:
        ...
        if status != 200: raise GatewayError(f"provider_read_http_{status}")
        return payload
    def _list_all(self, path: str, key: str, *, max_pages: int = 20) -> dict[str, Any]:
        rows: list[Any] = []
        token: str | None = None
        for _ in range(max_pages):
            sep = "&" if "?" in path else "?"
            page_path = path if not token else f"{path}{sep}pageToken={urllib.parse.quote(token, safe='')}"
            payload = self._read(page_path)
            rows.extend(payload.get(key) or [])
            token = str(payload.get("nextPageToken") or "") or None
            if not token: return {key: rows, "pagination_complete": True}
        raise GatewayError(f"provider_{key}_pagination_incomplete")
    def list_sources(self): return self._list_all("/sources?pageSize=100", "sources")
    def list_sessions(self): return self._list_all("/sessions?pageSize=100", "sessions")
    def get_session(self, sid): return self._read(f"/sessions/{urllib.parse.quote(sid)}")
    def list_activities(self, sid): return self._list_all(f"/sessions/{urllib.parse.quote(sid)}/activities?pageSize=100", "activities")
...
class GitHubClient:
    ...
    def _get(self,path: str) -> dict[str, Any]:
        ...
        except urllib.error.HTTPError as exc: raise GatewayError(f"github_read_http_{exc.code}") from exc
        except (TimeoutError,urllib.error.URLError) as exc: raise GatewayError(f"github_read_{type(exc).__name__}") from exc
    ...
    def artifact_names(self, *, max_pages: int = 5) -> list[str]:
        ...
        for page in range(1,max_pages+1):
            ...
            names.extend(str(row.get("name") or "") for row in rows if row.get("name") and not row.get("expired"))
