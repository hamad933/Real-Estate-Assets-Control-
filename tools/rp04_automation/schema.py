...
from .common import (
    ALLOWED_ACTIONS, BRANCH_RE, DIGEST_RE, DOMAIN_RE, ID_RE, MUTATION_ACTIONS,
    PROJECT_ID, READ_ACTIONS, RECONCILE_ACTIONS, REPOSITORY, SESSION_RE, SHA_RE,
...
def _require_domain(value: Any) -> str:
    ...
    if not DOMAIN_RE.fullmatch(text):
        raise ValidationError("invalid_write_domain")
    return text
...
def _require_branch(value: Any) -> str:
    ...
    if not BRANCH_RE.fullmatch(text):
        raise ValidationError("invalid_starting_branch")
    return text
...
def normalize_request(raw: dict[str, Any]) -> dict[str, Any]:
    ...
    allowed = {
        "schema_version", "request_id", "project_id", "controller_id", "lane",
        "logical_task_id", "action", "write_domain", "repository", "starting_branch",
        "expected_sha", "session_id", "expected_session_state", "expected_session_update_time",
        "expected_plan_id", "expected_plan_digest", "expected_plan_activity", "instruction_ref",
        "instruction_digest", "authority_ref", "authority_event", "target_request_id",
        "target_intent_identity", "prompt", "title", "require_plan_approval",
    }
    ...
    if out["project_id"] != PROJECT_ID or out["repository"] != REPOSITORY:
        raise ValidationError("project_or_repository_mismatch")
    if out["controller_id"] != "CENTRAL" or out["lane"] != "AUTOMATION":
        raise ValidationError("controller_or_lane_not_authorized_by_schema")
    if raw.get("starting_branch") not in (None, ""):
        out["starting_branch"] = _require_branch(raw["starting_branch"])
    ...
    if raw.get("expected_sha") not in (None, ""):
        ...
        if not SHA_RE.fullmatch(sha):
            raise ValidationError("invalid_expected_sha")
    ...
    for key in ("expected_plan_digest", "instruction_digest", "target_intent_identity"):
        if raw.get(key) not in (None, ""):
            ...
            if not DIGEST_RE.fullmatch(val):
                raise ValidationError(f"invalid_{key}")
            out[key] = val
    if raw.get("target_request_id") not in (None, ""):
        out["target_request_id"] = _require_id("target_request_id", raw["target_request_id"])
    if raw.get("prompt") not in (None, ""):
        prompt = str(raw["prompt"])
        if not prompt or len(prompt) > 20000:
            raise ValidationError("invalid_prompt")
        out["prompt"] = prompt
    if raw.get("title") not in (None, ""):
        out["title"] = str(raw["title"])[:200]
    out["require_plan_approval"] = bool(raw.get("require_plan_approval", True))
    if action in MUTATION_ACTIONS | RECONCILE_ACTIONS:
        if out["write_domain"] == "READ_ONLY":
            raise ValidationError("mutation_requires_write_domain")
        if not out.get("authority_event") and not out.get("authority_ref"):
            raise ValidationError("mutation_requires_authority_reference")
    if action == "create_session":
        for key in ("starting_branch", "expected_sha", "prompt"):
            if not out.get(key):
                raise ValidationError(f"create_session_requires_{key}")
    ...
    if action == "send_message":
        if not out.get("prompt"):
            raise ValidationError("send_message_requires_prompt")
        for key in ("expected_session_state", "expected_session_update_time"):
            if not out.get(key):
                raise ValidationError(f"send_message_requires_{key}")
    if action == "approve_plan":
        for key in ("expected_session_state", "expected_session_update_time", "expected_plan_digest", "expected_plan_activity"):
            if not out.get(key):
                raise ValidationError(f"approve_plan_requires_{key}")
